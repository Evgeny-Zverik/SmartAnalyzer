...
REGION_HINT_PATTERNS = (
    r"\b(?:[а-яё-]+\s+){0,3}(?:область|край|республика|автономный округ|автономная область)\b",
...
def build_case_law_stub_result(query: str) -> TenderAnalyzerResult:
    ...
    region_aliases: list[tuple[tuple[str, ...], str, str]] = [
        (("брянск", "брянской", "брянская"), "Брянская область", "А09"),
        (("москва", "московский", "московская"), "Москва", "А40"),
        (("санкт", "петербург", "ленинград"), "Санкт-Петербург", "А56"),
        (("калуга", "калужской", "калужская"), "Калужская область", "А23"),
    ]
    matched_regions = [(label, prefix) for aliases, label, prefix in region_aliases if any(alias in query_lower for alias in aliases)]
    if not matched_regions:
        matched_regions = [("Москва", "А40"), ("Брянская область", "А09")]
