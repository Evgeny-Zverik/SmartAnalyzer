...
export default function ResetPasswordPage() {
  ...
  const canSubmit = password.length >= 8 && passwordsMatch && !!token && !loading;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(parseApiError(err).message || "Не удалось обновить пароль");
    } finally {
      setLoading(false);
    }
  }
      ...
      <main className="w-full">
        <div className="rounded-3xl border border-rose-200 bg-white/85 p-8 text-center shadow-[0_30px_80px_rgba(15,23,42,0.1)] backdrop-blur">
          ...
          <Link
            ...
            className="mt-5 inline-flex rounded-xl bg-zinc-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-zinc-800"
          >
            Запросить заново
          </Link>
        </div>
