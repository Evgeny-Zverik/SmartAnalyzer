...
## Экономика кредитов и токенов

### Пакеты кредитов
Текущие пакеты задаются в `backend/app/api/v1/billing.py`.

| Пакет | Цена | Кредиты | Цена 1 кредита |
|---|---:|---:|---:|
| Start | 490 руб. | 2 000 | 0,245 руб. |
| Pro | 1 490 руб. | 7 000 | 0,213 руб. |
| Business | 3 990 руб. | 20 000 | 0,1995 руб. |

Для расчёта безопасности используется самый дешёвый для пользователя пакет
Business: `1 кредит = 0,1995 руб.`. После комиссий, налогового резерва,
серверов и запаса на бесплатные кредиты безопасный AI-бюджет принят как
`0,08 руб. на 1 кредит`.

### Себестоимость LLM-токенов
Тариф провайдера для модели `deepseek/deepseek-chat`:

- входящие токены: `36 руб. / 1 млн`
- исходящие токены: `149 руб. / 1 млн`

Формула себестоимости запуска:

```text
token_cost_rub = tokens_in * 36 / 1_000_000 + tokens_out * 149 / 1_000_000
```

Формула токенного списания в кредитах:

```text
token_credits = ceil(token_cost_rub / 0.08)
```

Итоговое списание за запуск:

```text
credits_charged = max(fixed_tool_credits, token_credits)
```

Это сохраняет понятную минимальную цену для пользователя, но защищает проект от
очень больших документов и длинных ответов LLM. Если фактическая токенная цена
оказалась выше фиксированной цены инструмента, backend спишет больше кредитов и
сохранит фактическую сумму в `usage_logs.credits_charged`.

### Текущие минимальные цены инструментов
Минимальные списания задаются в `backend/app/services/usage.py`.

| Инструмент | Минимум кредитов |
|---|---:|
| `spelling-checker` | 30 |
| `key_points` | 30 |
| `dates_deadlines` | 30 |
...
AI-себестоимости примерно такие:

| Минимум кредитов | AI-бюджет | Примерный лимит токенов |
|---:|---:|---:|
| 30 | 2,39 руб. | 34 тыс. |
| 40 | 3,19 руб. | 45 тыс. |
| 50 | 3,99 руб. | 57 тыс. |
| 60 | 4,79 руб. | 68 тыс. |
| 80 | 6,38 руб. | 91 тыс. |
| 120 | 9,58 руб. | 137 тыс. |
| 140 | 11,17 руб. | 160 тыс. |
| 160 | 12,77 руб. | 183 тыс. |

Если запуск выходит за эти ориентиры, динамическое списание по токенам должно
поднять `credits_charged` выше минимальной цены инструмента.
